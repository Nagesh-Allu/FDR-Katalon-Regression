...
WebUI.waitForElementPresent(findTestObject('FDR-Receipt/FDR-Receipt-Header/Edit/3dots_icon'), 4)
...
Uifunctions.clearElementText(findTestObject('FDR-Receipt/FDR-Receipt-Checkline/View/receipt_ID'))

WebElement elementri = WebUiCommonHelper.findWebElement(findTestObject('FDR-Receipt/FDR-Receipt-Checkline/View/receipt_ID'), 30)

elementri.clear()

WebUI.sendKeys(findTestObject('FDR-Receipt/FDR-Receipt-Checkline/View/receipt_ID'), ReceiptId)

WebUI.waitForElementPresent(findTestObject('FDR-Receipt/FDR-Receipt-Checkline/Edit/button_Save'), 30)

WebUI.click(findTestObject('FDR-Receipt/FDR-Receipt-Checkline/Edit/button_Save'))

if (WebUI.getText(findTestObject('FDR-Receipt/FDR-Receipt-Checkline/View/receiptId_Added')) == findTestData('Receipt/FDR-EditCheckLineTestData').getValue(
    1, 1)) {
    KeywordUtil.markPassed('ReceiptId updated successfully')
} else {
    (WebUI.getText(findTestObject('FDR-Receipt/FDR-Receipt-Checkline/View/receiptId_Added')) != findTestData('Receipt/FDR-EditCheckLineTestData').getValue(
        1, 1)).call({ 
            KeywordUtil.markFailedAndStop('ReceiptId not updated successfully')
        })
}
