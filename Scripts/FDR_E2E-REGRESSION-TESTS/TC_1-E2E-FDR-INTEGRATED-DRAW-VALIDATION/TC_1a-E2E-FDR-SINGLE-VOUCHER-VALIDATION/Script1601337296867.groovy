 ...
 * 	1) Logic search updated for Voucher Number
 *  2) Updated datasheet for few Vouchers
 */
...
ExcelData vocuhersExcelData = TestDataFactory.findTestData('Regression-Voucher/E2E Regression-Single_Voucher')
...
for (int i = 1; i <= numOfSheets-1; i++) {
	...
	def processorVoucherNumber = null
	processorVoucherNumber = WebUI.getText(findTestObject('FDR-VoucherEntry/VoucherHeader/voucherNum'))
	voucherIds.add(processorVoucherNumber)
...
for (int i = 0; i < voucherIds.size(); i++) {
	VF.validateToApprove(voucherIds[i])
}
